const map_leaflet = L.map("map").setView([53.9, 27.55], 15);
...
rootEl.style.setProperty("--mapTranslate", `${infoContainer.clientHeight * -1}px`);
...
const dataTaking = (data) => {
  ...
  L.marker([IPlat, IPlng], { icon: mapPosIcon }).addTo(map_leaflet).bindPopup(`${IPtaken_ip} located here`);
  return;
};
